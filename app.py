...
with st.sidebar:
    ...
    st.caption("Tip: keep CSV to ~300 compositions for highly interactive runs; use caching for repeated runs.")

    st.header("Speed Mode")
    speed_mode = st.radio(
        "Prediction grid",
        options=["Full accuracy (100-2000, step 100)", "Fast interactive (~2x faster)"],
        index=1,
        help="Fast mode uses a coarser temperature grid for quicker iteration.",
    )

    if speed_mode == "Fast interactive (~2x faster)":
        tmin, tmax, tstep = 300, 1900, 200
    else:
        tmin, tmax, tstep = 100, 2000, 100

    st.caption(f"Active grid: {tmin} to {tmax} K, step {tstep} K ({((tmax - tmin)//tstep) + 1} points/composition)")
...
with tab1:
    st.subheader("Predict kappa vs temperature")
    ...
    if run_kappa:
        try:
            ...
            with st.spinner("Running kappa featurization + prediction..."):
                pred_df, failed_df = _cached_predict_kappa(tuple(compositions), kappa_model_dir, tmin, tmax, tstep)
...
with tab2:
    ...
    if st.button("Rank lowest-kappa candidates"):
        try:
            ...
            available_t = set(st.session_state.kappa_predictions["T"].unique().tolist())
            if int(target_t) not in available_t:
                raise ValueError(
                    f"T={int(target_t)} K is not in current prediction grid. Available temperatures: {sorted(available_t)}"
                )
        ...
        if run_cte:
            try:
                ...
                with st.spinner("Running CTE featurization + prediction..."):
                    cte_df, failed_df = _cached_predict_cte(tuple(compositions), cte_model_dir, tmin, tmax, tstep)
                st.session_state.cte_predictions = cte_df
