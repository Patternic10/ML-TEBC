...
def load_predictions(input_csv):
    ...
    required = {"Composition", "T", "GBR_Pred"}
...
def rank_at_T(df, target_T, top_n, ascending):
    df_T = df[df["T"] == target_T]

    if df_T.empty:
        raise ValueError(f"No rows found at T = {target_T}")

    ranked = (
        df_T.sort_values("GBR_Pred", ascending=ascending)
        .head(top_n)
        [["Composition", "T", "GBR_Pred"]]
    )
    return ranked


def rank_aggregate(df, mode, top_n, ascending):
    grouped = df.groupby("Composition")["GBR_Pred"]
    ...
    ranked = ranked.sort_values("Score", ascending=ascending).head(top_n)
    return ranked
...
def plot_results(full_df, selected):
    ...
    for comp in comps:
        ...
        plt.plot(grp["T"], grp["GBR_Pred"], marker="o", label=comp)
...
def main():
    ...
    parser.add_argument("--mode", required=True,
                        choices=["highest_at_T", "lowest_at_T",
