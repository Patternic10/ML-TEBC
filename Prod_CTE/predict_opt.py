...
def load_artifacts(model_dir: str):
    ...
    model = joblib.load(os.path.join(model_dir, "gbr_model.joblib"))
    features = joblib.load(os.path.join(model_dir, "features.joblib"))
    magpie_features = joblib.load(os.path.join(model_dir, "magpie_features.joblib"))

    return scaler, model, features, magpie_features
...
def featurize_compositions_batch(compositions, magpie_features):
    """
    Featurize all compositions in one batch using your matminer pipeline.
    Keeps only the MagpieData columns expected by the trained model.
    """
    ...
    # keep only rows without featurization errors
    ...
    missing = [c for c in magpie_features if c not in feat_df.columns]
    if missing:
        raise ValueError(
            "Generated features do not match the saved training Magpie columns.\n"
            f"First few missing columns: {missing[:10]}"
        )

    feat_df = feat_df[["Composition"] + magpie_features].copy()
...
def predict_batch(
    ...
    feat_df, failed_df = featurize_compositions_batch(compositions, magpie_features)
    ...
    pred_df["GBR_Pred"] = model.predict(X_scaled)

    pred_df = pred_df[["Composition", temp_col, "GBR_Pred"]]
    return pred_df, failed_df


def make_plot(pred_df, temp_col="T", pred_col="GBR_Pred", output_plot=None):
    ...
    plt.ylabel("CTE")
    plt.title("Predicted CTE vs T (Gradient Boosting)")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Batch predict CTE vs temperature from composition using a saved GBR model."
    )
...
if __name__ == "__main__":
    ...
    scaler, model, features, magpie_features = load_artifacts(args.model_dir)
